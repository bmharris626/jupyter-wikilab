...
After editing Python files (like `jupyterhub_wikilab/routes.py`):

```bash
python -m py_compile jupyterhub_wikilab/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `jupyterhub_wikilab/*.py` files):
...
- Backend processing in `jupyterhub_wikilab/routes.py`
...
- **✅ Do**: `jupyterhub_wikilab/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'jupyterhub_wikilab:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyterhub_wikilab:open-panel';
  export const refreshData = 'jupyterhub_wikilab:refresh-data';
}
...
- Backend routes: `jupyterhub_wikilab/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `jupyterhub_wikilab/` directory
...
1. **Read the backend first** — Check `jupyterhub_wikilab/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `jupyterhub_wikilab/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "jupyterhub_wikilab", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `jupyterhub_wikilab/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `jupyterhub_wikilab/`)
...
- **Output**: Creates files in `lib/` and `jupyterhub_wikilab/labextension/`
...
**After editing Python** (files in `jupyterhub_wikilab/`):
...
- Ask user to search for the extension ID (`jupyterhub_wikilab`) to see if it loaded
...
ls -la jupyterhub_wikilab/labextension/  # Should contain bundled extension
```
...
- Backend routes: `jupyterhub_wikilab/routes.py`
- Server extension config: `jupyter-config/server-config/jupyterhub_wikilab.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `jupyterhub_wikilab/routes.py`
...
**❌ Don't**: Use absolute imports like `from jupyterhub_wikilab.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'jupyterhub_wikilab:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyterhub_wikilab:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyterhub-wikilab-ClassName`
- **API routes** (in `jupyterhub_wikilab/routes.py`): `url_path_join(base_url, "jupyterhub_wikilab", "endpoint")`
